...
for i, methode in enumerate(methoden):  # Schleife über jede Methode (z. B. Phishing-Art)
    # Bestimme Start- und Endposition für den Datenblock dieser Methode
    start_index = startzeile + i * blockgröße
    end_index = start_index + blockgröße
    # Extrahiere den Block von Angriffen für die aktuelle Methode
    block = tabelle.iloc[start_index:end_index]
    # Wandle die relevanten Spalten (Prinzipien) in ein NumPy-Array um
    daten = block[header_spalten].to_numpy()

    for j, spaltenname in enumerate(header_spalten):  # Schleife über jedes Prinzip (Spalte)
        # Berechne den Median der Prinzip-Ausprägungen und speichere ihn in der Heatmap-Tabelle
        # → Der Median gibt an, wie stark das Prinzip in dieser Methode typischerweise vorkommt
        heatmap_data.loc[methode, spaltenname] = np.median(daten[:, j])
        
# Erstelle eine neue Figur mit fester Größe für die Heatmap
plt.figure(figsize=(12, 6))

# Zeichne die Heatmap mit farblicher Darstellung der Medianwerte
# - annot=True: Zahlen in Zellen anzeigen
# - fmt=".2f": Zahlen mit 2 Dezimalstellen formatieren
# - cmap="YlOrRd": Farbschema von Gelb über Orange zu Rot
...
# Grafik als PNG-Bild abspeichern mit hoher Auflösung und vollständigem Rahmen
